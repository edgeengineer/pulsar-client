        ...
        while mutableContext.attemptNumber < mutableContext.retryPolicy.maxRetries {
            ...
            do {
                ...
                if mutableContext.attemptNumber > 1 {
                    logger.info("Operation \(mutableContext.operation) succeeded after \(mutableContext.attemptNumber) attempts")
                }
                return result
            } catch {
                lastError = error
                ...
                case .retry:
                    ...
                    await delayRetry(delay, operation: mutableContext.operation, attempt: mutableContext.attemptNumber)
                    
                case .retryAfter(let customDelay):
                    await delayRetry(customDelay, operation: mutableContext.operation, attempt: mutableContext.attemptNumber)
                }
            }
        }
        
        // All retries exhausted
        logger.error("Operation \(mutableContext.operation) failed after \(mutableContext.attemptNumber) attempts")
