...
actor ConsumerImpl<T>: ConsumerProtocol where T: Sendable {
  ...
  func handleIncomingMessage(
    _ commandMessage: Pulsar_Proto_CommandMessage, payload: Data,
    metadata: Pulsar_Proto_MessageMetadata
  ) async {
    logger.info(
      "Consumer \(id) received message: ledger=\(commandMessage.messageID.ledgerID), entry=\(commandMessage.messageID.entryID)"
    )
    ...
    do {
      ...
      let message = Message(
        id: MessageId(
          ...
          partition: commandMessage.messageID.hasPartition
            ? commandMessage.messageID.partition : -1,
          batchIndex: commandMessage.messageID.hasBatchIndex
            ? commandMessage.messageID.batchIndex : -1,
          topic: topicName
        ),
        value: value,
  ...
  private func requestMoreMessages() async {
    ...
    if isFirstFlow {
      ...
      guard needed > 0 else {
        logger.debug(
          "No permits to request - current: \(permits), queue size: \(configuration.receiverQueueSize)"
        )
        return
      }
      permitsToRequest = needed
      logger.info(
        "Consumer \(id) requesting \(permitsToRequest) additional permits (current: \(permits))")
    }
    ...
    do {
      ...
      logger.info(
        "Successfully sent FLOW command for \(permitsToRequest) permits, total permits now: \(permits)"
      )
    } catch {
  ...
  private func determineTopicName(
    from commandMessage: Pulsar_Proto_CommandMessage, metadata: Pulsar_Proto_MessageMetadata
  ) -> String {
