...
public protocol SendChannelProtocol<T>: Sendable where T: Sendable {
    ...
    func sendAsync(_ message: T, metadata: MessageMetadata?, completion: @escaping @Sendable (Result<MessageId, Error>) -> Void) async
...
public actor SendChannel<T>: SendChannelProtocol where T: Sendable {
    ...
    private var pendingMessages: [(message: T, metadata: MessageMetadata?, completion: @Sendable (Result<MessageId, Error>) -> Void)] = []
    ...
    public func sendAsync(_ message: T, metadata: MessageMetadata?, completion: @escaping @Sendable (Result<MessageId, Error>) -> Void) async {
        guard !isClosed else {
    ...
    private func processMessages() async {
        while !isClosed {
            if let item = pendingMessages.first {
                ...
                do {
                    let messageId = try await producer.send(item.message, metadata: item.metadata ?? MessageMetadata())
